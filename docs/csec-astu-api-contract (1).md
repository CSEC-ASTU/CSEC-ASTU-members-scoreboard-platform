...
```json
{
  ...
  "display_score": 340,
  "career_score": 890,
  "permissions": ["approve_task:division:<uuid>", "manage_tasks:division:<uuid>"]
}
```
...
| Method & path | Purpose | Auth |
|---|---|---|
| `GET /members` | List members — filters: `division_id`, `role`, `is_active`, `search` (name/email) | officer (scoped to own division unless club-wide) or president |
| `GET /members/{id}` | Get one member's full profile + scores | self, an officer with visibility into that member, or president |
...
```json
{
  ...
  "score_cap": 2500,
  "items": [
    { "rank": 1, "member_id": "uuid", "full_name": "string", "division_id": "uuid",
      "display_score": 2500, "career_score": 4200, "badge": "platinum" },
    { "rank": 2, "member_id": "uuid", "full_name": "string", "division_id": "uuid",
      "display_score": 1870, "career_score": 1870, "badge": null }
  ]
}
```
...
3. `GET /auth/google/callback` looks up the returned email against `members.email`:
   - **Match, `google_id IS NULL`** → this is the student's first login. Their account is *claimed*: `google_id` is set, `first_login_at` is stamped, cookies are issued, they land on the dashboard.
