...
2. Backend looks up the Google account's email against an **already-imported** `members` row (see "Registration" below) — it never creates a new row on login. First-ever login for that row *claims* it (sets `google_id`); every login after that is a normal lookup. An email with no matching row is rejected outright.
3. Both a short-lived JWT and a longer-lived refresh token are set as `httpOnly`, `Secure`, `SameSite=Lax` cookies — never exposed to frontend JS.
...
**Registration — CSV import, not self-signup:** real registration happens through the club's existing Google Form, not the platform. An officer exports the Form responses as CSV and imports it (`full_name, email, department, joining_year, division` columns) — this creates the `members` row with `google_id` still null. The student's first Google login is what claims that row. This keeps the roster officer-curated (matches "students will have login only access") and means there's no self-serve onboarding step to build — the CSV already carries what onboarding used to collect. Full mechanics, including what happens on an unmatched login, are in the companion API contract's §13.
